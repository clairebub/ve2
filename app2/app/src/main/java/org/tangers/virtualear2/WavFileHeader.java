...
public class WavFileHeader {
    ...
    public WavFileHeader(int sampleRate, int numChannels, int numSamples) {
    ...
    public static byte[] getHeader(int sampleRate, int numChannels, int numSamples) {
        return new WavFileHeader(sampleRate, numChannels, numSamples).mHeader;
    }
    ...
    private void setHeader() {
        ...
        header[offset++] = (byte)((size >> 24) & 0xFF);
        if (offset != 44) {
            throw new IllegalStateException(String.format("Unexpected WavFileHeader offset: ", offset));
        }
        mHeader = header;
    }
}
